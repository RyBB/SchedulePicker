chrome.extension.onMessage.addListener(function (request, sender, sendResponse) {
	...
	getSchedule(date).done(function (xml) {
		...
		$(xml).find("schedule_event").each(function () {
			...
			console.log($(this).children("when").children("datetime")[0]);
			console.log($(this).find("start"));
			$test = null//$(this).children().children("datetime")[0];
			if($test){
			...
			console.log(schedule[index].event_type);
			index = index + 1;
		});
		console.log("----------------------------------------------------");
